...
const UsersTable = ({ mockData }) => {
  ...
  function handleDelete() {
    if (rank === "admin") {
      if (delete_id > 0) {
        axios
          .delete(`http://localhost:3000/users/delete-user`, { data: { id: delete_id } })
          .then((res) => {
            if (res.status === 201) {
      ...
      <div className={mobileNav ? "hidden" : "flex flex-col justify-center items-center w-full h-full"}>
        ...
        <div className="grid-container">
          ...
          <div className="grid-item-title">
            <h1 className="text-center text-4xl text-white bg-[#415191] rounded-t-lg border-b-[1px] border-black p-4">Users Table</h1>
          </div>
          ...
          <div className="grid-item-buttons grid-cols-3">
            ...
            <form onSubmit={handleDelete} className="grid grid-cols-2 grid-rows-1">
              <button type="submit" className="col-[1_/_1] text-white border-2 px-6 py-3 flex justify-center items-center hover:bg-red-600 hover:border-red-600">
                Delete
              </button>
